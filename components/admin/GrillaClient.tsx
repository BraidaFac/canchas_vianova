...
type Cancha = { id: number; nombre: string; tipo: string; jugadores: number; activa: boolean };
...
function estadoBadge(estado: string, esFijo: boolean) {
  const estadoEl = estado === "confirmada"
    ? <Badge variant="default" className="text-[10px] px-1 py-0 bg-green-600">CONF</Badge>
    : estado === "completada"
      ? <Badge variant="default" className="text-[10px] px-1 py-0 bg-emerald-700">COMP</Badge>
      : estado
        ? <Badge variant="secondary" className="text-[10px] px-1 py-0">PEND</Badge>
        : null;

  if (esFijo) {
    return (
      <div className="flex gap-0.5 flex-wrap">
        <Badge variant="outline" className="text-[10px] px-1 py-0 border-violet-400 text-violet-600">FIJO</Badge>
        {estadoEl}
      </div>
    );
  }
...
export default function GrillaClient({
  ...
  const [modalCreate, setModalCreate] = useState<{ canchaId?: number; turnoId?: number; fecha?: string } | null>(null);
  ...
  function getSlotA(canchaId: number, turnoId: number) {
    const r = reservas.find((r) => r.cancha_id === canchaId && r.turno_id === turnoId);
    if (r) return { reserva: r, esFijo: !!r.recurrente_id };
    const f = fijosDelDia.find((f) => f.cancha_id === canchaId && f.turno_id === turnoId);
    if (f) return { fijo: f, esFijo: true };
    return null;
  }

  // Vista Lista: slot para fecha+dia específico
  function getSlotLista(canchaId: number, turnoId: number, fechaStr: string, diaSemana: number) {
    const r = allReservas.find((r) => r.cancha_id === canchaId && r.turno_id === turnoId && r.fecha === fechaStr);
    if (r) return { reserva: r, esFijo: !!r.recurrente_id };
    const f = todosLosFijos.find((f) => f.cancha_id === canchaId && f.turno_id === turnoId && f.dia_semana === diaSemana);
    if (f) return { fijo: f, esFijo: true };
    return null;
  }
    ...
    <div className="flex flex-col flex-1 min-h-0">
      ...
      <div className="flex flex-wrap items-center justify-between gap-x-2 gap-y-2 px-4 py-2 border-b border-border bg-card sticky top-0 z-10">
        ...
        {vista === "A" && (
          <div className="flex items-center gap-1">
            <Button variant="ghost" size="icon" className="h-8 w-8 shrink-0" onClick={() => navegarFecha(-1)}>
              <ChevronLeft size={16} />
            </Button>
            <DatePicker
              value={fecha}
              ...
              className={cn(
                ...
                isToday && "border-primary/40 bg-primary/5 text-primary hover:bg-primary/10"
              )}
              isToday={isToday}
              showDayName
            />
        ...
        {vista === "3" && (
          <div className="flex items-center gap-2">
            ...
            <ToggleGroup
              ...
              onValueChange={(v) => { if (v) setDiasAMostrar(Number(v)); }}
              className="border border-border rounded-md"
            >
              <ToggleGroupItem value="7" className="h-8 px-2.5 text-xs">7d</ToggleGroupItem>
              <ToggleGroupItem value="10" className="h-8 px-2.5 text-xs">10d</ToggleGroupItem>
              <ToggleGroupItem value="15" className="h-8 px-2.5 text-xs">15d</ToggleGroupItem>
            </ToggleGroup>
          </div>
        )}
                  ...
                  {canchas.map((cancha) => {
                    ...
                    if (!slot) return (
                      <td
                        ...
                        onClick={() => setModalCreate({ canchaId: cancha.id, turnoId: turno.id, fecha })}
                      >
                        <span className="text-muted-foreground/30">—</span>
                      </td>
                    );
                    const cliente = "reserva" in slot ? slot.reserva?.clientes : slot.fijo?.clientes;
                    return (
                      <td
                        key={cancha.id}
                        className={cn(
                          "h-10 align-middle px-2 py-1.5 border-r border-b border-border last:border-r-0 cursor-pointer",
                          slot.esFijo
                            ? "bg-violet-50 dark:bg-violet-950/30 hover:bg-violet-100 dark:hover:bg-violet-950/50"
                            : "reserva" in slot && slot.reserva?.estado === "completada"
                              ? "bg-emerald-50 dark:bg-emerald-950/30 hover:bg-emerald-100 dark:hover:bg-emerald-950/50"
                            : "reserva" in slot && slot.reserva?.estado === "confirmada"
                      ...
                      >
                        <div className="flex flex-col gap-0.5">
                          {"reserva" in slot ? estadoBadge(slot.reserva?.estado ?? "", slot.esFijo) : estadoBadge("", true)}
                          <span className="font-medium truncate max-w-[80px]">{cliente?.nombre ?? "—"}</span>
                                                  </div>
                      </td>
                    );
                  })}
          ...
          <table className="w-full text-xs border-collapse" style={{ minWidth: `${80 + diasAMostrar * 100}px` }}>
            <thead className="sticky top-0 bg-muted/60 z-10">
              <tr>
                ...
                {diasCols.map((d) => (
                  <th key={d.fechaStr} className={cn(
                    "px-2 py-1.5 text-center font-medium border-b border-r border-border last:border-r-0 min-w-[90px]",
                    d.esHoy && "bg-primary/5"
                  )}>
                    <div className={cn("capitalize text-[11px]", d.esHoy ? "text-primary font-semibold" : "text-foreground")}>
                      {d.diaNombre} {d.diaNum}
                    </div>
                    <div className="text-[10px] font-normal text-muted-foreground capitalize">{d.mes}</div>
                    {d.esHoy && <div className="text-[9px] font-bold text-primary uppercase tracking-wide">hoy</div>}
                  </th>
                ))}
              </tr>
            </thead>
            <tbody>
              {turnos.map((turno) => (
                <tr key={turno.id} className="hover:bg-accent/30">
                  ...
                  {diasCols.map((d) => {
                    const slot = getSlotLista(Number(canchaSeleccionada), turno.id, d.fechaStr, d.diaSemana);
                    if (!slot) return (
                      <td
                        key={d.fechaStr}
                        className={cn(
                        ...
                        onClick={() => setModalCreate({ canchaId: Number(canchaSeleccionada), turnoId: turno.id, fecha: d.fechaStr })}
                      >
                        <span className="text-muted-foreground/30">—</span>
                      </td>
                    );
      ...
      {modalCreate !== null && (
        <ReservaModal
          ...
          onSuccess={() => { setModalCreate(null); router.refresh(); }}
          canchas={canchas}
          turnos={turnos}
          precios={precios}
          prefill={modalCreate}
